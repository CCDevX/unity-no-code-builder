  ...
  function initBuilder() {
    ...
    function loadProjectState() {
      ...
      if (project && project.components) {
        // Supprimer l'état vide initial
        const emptyState = dropArea.querySelector(".empty-state");
        if (emptyState) {
          emptyState.remove();
        }

        // Recréer chaque composant
        project.components.forEach((compState) => {
          // Extraire le vrai type à partir du HTML stocké
          let componentType = compState.type;
          if (compState.type === "preview") {
            if (compState.content.includes("unity-text"))
              componentType = "text";
            else if (compState.content.includes("unity-btn"))
              componentType = "button";
            else if (compState.content.includes("unity-input"))
              componentType = "input";
            else if (compState.content.includes("unity-helpbox"))
          ...
          // Ajouter les événements de sélection sur l'élément intérieur
